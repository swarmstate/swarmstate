...
- **M5 (CrewAI adapter + Redis backend)** ✅ — `RedisStore` (persistent, msgpack
  wire-format, drop-in checkpointer backend) and `SwarmStateStorage` (CrewAI-compatible
  memory backed by a shared `Store`).
